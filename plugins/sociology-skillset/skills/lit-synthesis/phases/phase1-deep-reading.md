...
|------|-------------|-------|--------------|
| **Local library** | Papers in library/ with annotations or markdown files | Opus | Read local markdown, leverage highlights |
| **Docling** | Batch processing PDFs, no annotations | Haiku | Convert PDF→markdown, spawn reading agents |

You can mix modes: use local library files for key theoretical papers you've annotated, Docling for the rest.
...
- Confirm the markdown file exists in `library/` (check `md_path` in references.bib)
- Check for any inline annotations or highlights in the markdown
...
```yaml
---
citation_key: smithCulturalFrames2020  # From database.json or references.bib (preferred)
openalex_id: W2123456789    # From lit-search
...
**How to populate `citation_key`**: Check `database.json` first (generated by lit-search). If the paper is in `references.bib`, use its BibTeX key directly. As a last resort, generate one: first author last name (lowercase) + up to 3 significant title words (capitalized) + year (e.g., `kirkCulturalMechanisms2011`).
...
**Why identifiers matter**: The frontmatter identifiers let you reliably match notes back to source papers across the workflow—from lit-search database through lit-synthesis to argument-builder citations. The `citation_key` is especially important because writing skills use `[@citationKey]` Pandoc syntax, and bibliography-builder looks up items by `citationKey` in `references.bib`.
...
Add tags to notes using citation keys when available:
- `[cites: kirkCulturalMechanisms2011]`
- `[contradicts: desmondEvictedPovertyProfit2016]`
- `[same-concept-as: smithCulturalFrames2020]`
...
## Using the Local Library Effectively

### Retrieving Full Text

Look up the paper's `citation_key` in `references.bib` to find the `md_path` field, then read that markdown file directly.

### Reading Annotations

If the user has annotated the PDF, those annotations may appear as inline comments or highlights in the converted markdown file.

### Searching for Connections

Use Grep to search across `library/*.md` files for a concept or author name to find related passages across papers.
...
1. **reading-notes/*.md** - One file per paper (with identifier frontmatter)
   ...
   - Frontmatter: Must include at least one of: `citation_key`, `openalex_id`, `doi` (prefer `citation_key` matching references.bib)
