...
**How to populate `citation_key`**: Check `database.json` first (generated by lit-search). If importing from Zotero, use `data.citationKey` from the item metadata. As a last resort, generate one: first author last name (lowercase) + up to 3 significant title words (capitalized) + year (e.g., `kirkCulturalMechanisms2011`).
...
1. **reading-notes/*.md** - One file per paper (with identifier frontmatter)
   ...
   - Frontmatter: Must include at least one of: `citation_key`, `openalex_id`, `doi`, `zotero_key` (prefer `citation_key`)
